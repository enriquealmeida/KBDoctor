...
namespace Concepto.Packages.KBDoctor
{
    static class KbStats
    {
        public static void CountGeneratedByPattern()
        {
            ...
            // string title = "KBDoctor - KBStats. Count objects generated by patterns";
            ...
            //IndexP = Objcount == 0 ? 0 : (ObjNoGenerate * 100 / Objcount);
            //writer.AddTableData(new string[] { "Generated='false' / objects", IndexP.ToString() + " %", "Generate=False =" + ObjNoGenerate.ToString() + " Obj.Gen=" + Objcount.ToString() });
        ...
        public static void ListObjDiffMasterPage2()
        {
            ...
            KBDoctorXMLWriter writer = new KBDoctorXMLWriter(outputFile, Encoding.UTF8);
            writer.AddHeader(title);
            //    writer.AddTableHeader(new string[] { "Type", "Object",  "Descripcion"});

            //   foreach (KBObject obj in kbserv.CurrentModel.Objects.GetAll())
            //      {
            //         writer.AddTableData(new string[] {obj.TypeDescriptor.ToString(), obj.Name, obj.Description});
            //     }
            /*
           KBCategory mainCategory = KBCategory.Get(kbserv.CurrentModel, "Main Programs");
           foreach (KBObject obj in mainCategory.AllMembers)
           {
               objLocation = (string)obj.GetProperty("AppLocation").Value;

               if (objLocation == null)
                   objLocation = "*None";

               if ((objLocation != null) && (!locationString.Contains(objLocation)))
                        {
                            output.AddLine("Calculating location " + objLocation + " references  ");
                            locationString = locationString + " " + objLocation;
                            dic.Add(objLocation, CreatLocationCollection(objLocation));
                        }
           }
               ...
               foreach (KBObject obj in mainCategory.AllMembers)
               {
                   ...
                   if (objLocation == null)
                            objLocation = "*None";
                   List2(obj,objLocation,dic,writer);
            ...
            foreach (KBObject obj in kbserv.CurrentModel.Objects.GetAll())
            {
                if (obj != null)
                    if (Functions.isRunable(obj) && ObjectsHelper.isGenerated(obj) && (obj.LastUpdate >= limite || obj.Timestamp >= limite))
                    {
                       ...
                       // if (objName == "DPFuturosBarcosANCAPSinMIC")
                       //     output.AddWarningLine(objName + "TimeStamp=" + obj.Timestamp.ToString() + " VersionDate=" + obj.VersionDate.ToString() + " LastCommited=" +  obj.LastCommitedDate.ToString() + " LastUpdate=" +  obj.LastUpdate.ToString());
        ...
        //public static void InitializeAppLocation()
        //{
        ...
        /*
        XmlNode nulo = null; // new XmlNode();

        //Display all the book titles.
        string objName = doc.SelectSingleNode("ObjectSpec/Object/ObjName").InnerText;


        string generator = doc.SelectSingleNode("ObjectSpec/Generator/GenId").InnerText;
        //string backupPath = directoryElement.GetAttribute("value");
       // output.AddLine(String.format("GEN%1_%2",generator,objName));
        output.AddText("GEN" + generator + "_" + objName);

        ExtraigoNodoLevel(output, doc, "Level");
        /*
        elemList = doc.GetElementsByTagName("ImplicitForEach");
        for (int i = 0; i < elemList.Count; i++)
        {
           // output.AddLine(elemList[i].InnerXml);
            doc.RemoveAll();
        }
        if (doc.InnerXml.Contains("Table"))
                 output.AddLine(doc.InnerXml);
             
    }

    private static void ExtraigoNodoLevel(IOutputService output, XmlDocument doc, string TagName2)
    {
        string tables = "";
        string attributes = "";

        XmlNodeList elemList = doc.GetElementsByTagName(TagName2);

            for (int i = 0; i < elemList.Count; i++)
            {
                if (elemList[i].InnerXml.Contains(TagName2))
                {
                    XmlDocument doc2 = new XmlDocument();
                    doc2.ImportNode( elemList[i], true);
                    ExtraigoNodoLevel(output, doc2 , TagName2);
                }
                else
                {
                tables = ExtractTAG(elemList[i].OuterXml, "TableName");
                attributes = ExtractTAG(elemList[i].OuterXml, "AttriName");
                doc.RemoveAll();
            }
        }
        output.AddText(" Tables = " + tables);
        output.AddText(" Attributes = " + attributes);
        output.AddLine("==");
    }

    private static string ExtractTAG(string xmlString, string TAG)
    {
        ...
        for (int i = 0; i < elemList.Count; i++)
        {
            values += " " + elemList[i].InnerText;
        }
        return values;
    }

    private static string ExtractTables(XmlNode xmlNode)
    {
        ...
        public static void ObjectsWINWEB()
        {
            ...
            foreach (KBObject obj in kbserv.CurrentModel.Objects.GetAll())
            {
                ...
                if (obj is Procedure || obj is Transaction)
                {
                    ...
                    {
                        count += 1; // writer.AddTableData(new string[] { obj.Name, genType.ToString(), "" });
                    }
                    if (count > 1)
                    {
                        KBObjectCollection objColl = new KBObjectCollection();
                        ...
                        foreach (EntityReference reference in obj.GetReferences())
                        {
                            ...
                            if ((objRef != null) && (objRef is WorkPanel || objRef is WebPanel) && (reference.ReferenceType == ReferenceType.Hard)) //&& (objRef.TypeDescriptor.Name != "MasterPage") ) 
                            {
                                ...
                                // if (!mainss.Contains("Java Win") || !mainss.Contains("HMaster"))
        ...
        public static void KBInterfaces()
        {
            ...
            foreach (KBObject obj in kbserv.CurrentModel.Objects.GetAll())
            {
                ...
                if (obj != null)
                {
                    ...
                    /// string mainss = MainsOf(obj, objColl, "");
                    string mainss = "";
